...
urlpatterns = [
    ...
    url(r'^load/(?P<pk>[0-9]+)/(?P<height>[0-9]+)/pathname/(?P<path>.*)/demographics/(?P<countryName>.*)/(?P<countryCode>.*)/(?P<city>.*)/(?P<regionName>.*)/(?P<ip>.*)/(?P<lang>.*)/href/(?P<href>.*)$',
        logger_views.PageLoadView.as_view()),
    url(r'^click/(?P<pk>[0-9]+)/(?P<x>[0-9]+)/(?P<y>[0-9]+)/pathname/(?P<path>.*)/href/(?P<href>.*)$',
